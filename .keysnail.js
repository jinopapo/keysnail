...
}, "List closed tabs");// ここにコードを入力して下さい

plugins.options["tanything_opt.keymap"] = {
    "C-l"   : "prompt-toggle-edit-mode",
    // Tanything specific actions
    "C-q"     : "localClose",
    "C-n" : "prompt-next-completion", //次のアイテムに移動
    "C-p" : "prompt-previous-completion" //前のアイテム移動
};
//}}%PRESERVE%
// ========================================================================= //
...
key.setViewKey(['C-x', 'C-b'], function (ev, arg) {
    ext.exec("bmany-list-all-bookmarks", arg, ev);
}, 'bmany - ブックマークを一覧表示');

key.setViewKey(['C-x', 'e'], function (aEvent, aArg) {
    ext.exec("hok-start-foreground-mode", aArg);
}, 'Hit a Hint を開始', true);

key.setViewKey(['C-x', 'C-e'], function (aEvent, aArg) {
    ext.exec("hok-start-background-mode", aArg);
}, 'リンクをバックグラウンドで開く Hit a Hint を開始', true);

key.setViewKey(['C-x', 'M-e'], function (aEvent, aArg) {
...
key.setViewKey(['C-x', 't'], function (ev) {
    command.focusElement(command.elementsRetrieverTextarea, 0);
}, '最初のインプットエリアへフォーカス', true);
...
key.setViewKey('C-M-n', function (ev) {
    getBrowser().mTabContainer.advanceSelectedTab(1, true);
}, 'ひとつ右のタブへ');

key.setViewKey('C-M-p', function (ev) {
    getBrowser().mTabContainer.advanceSelectedTab(-1, true);
}, 'ひとつ左のタブへ');
...
key.setViewKey(['C-c', 'b'], function (ev) {
    BrowserBack();
}, '戻る');

key.setViewKey(['C-c', 'f'], function (ev) {
    BrowserForward();
}, '進む');
...
key.setCaretKey('C-e', function (aEvent, aArg) {
    ext.exec("hok-start-extended-mode", aArg);
}, 'HoK - 拡張ヒントモード', true);
